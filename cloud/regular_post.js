...
Parse.Cloud.define("fetchPost", async (request) => {
...
Parse.Cloud.define("addFakeData", async (request) => {
    var objectId = request.params.objectId;
    if (!objectId) {
        throw "please supply objectId"
    }else{
...
Parse.Cloud.define("posts", async (request) => {
    ...
    if (!appId) {
        ...
        if(type === "mySubscribed"){
            ...
            return querySubs.find().then(function(res){
                if (res.length > 0) {
                    var categories = [];
...
Parse.Cloud.define("addPost", async (request) => {
        ...
        if (youtubeId && type === "youtube") {
            ...
            query.first().then(function(objectExisting) {
                if (objectExisting) {
                    ...
                    save(object, push,undefined,undefined,fbUpload,undefined)
                }
            }, function(error) {
                throw error.message
            });
...
function save(object, push,isUpdate,oldCategory,fbUpload,oldFileName) {
            ...
            if (youtubeId && type === "youtube") {
                ...
                query.first().then(function(objectExisting) {
                    if (objectExisting && !(objectExisting.get("youtubeId") === youtubeId) ) {
                        ...
                        if(category)
                            save(object, push,true,oldCategory,fbUpload,oldFileName)
                        else
                            save(object, push,true,undefined,fbUpload,oldFileName)
                    }
                }, function(error) {
                    throw error.message
                });
...
Parse.Cloud.define("incrementPostView", async (request) => {
    var objectId = request.params.objectId;
    if (!objectId) {
        throw "please supply objectId"
    }else{
...
Parse.Cloud.define("forcePublishToFb", async (request) => {
    var objectId = request.params.objectId;
    if (!objectId) {
        throw "please supply objectId"
    }else{
        ...
        query.first().then(function(object) {
             ...
             if(object.get("media") && object.get("type") === "image" && app.get("facebookPostPath") && app.get("facebookAccessToken")){
                ...
                throw "Either fb settings are missing or post type is not image"
             }
...
Parse.Cloud.define("forceSendPush", async (request) => {
    var objectId = request.params.objectId;
    if (!objectId) {
        throw "please supply objectId"
    }else{
...
Parse.Cloud.define("requestPost", async (request) => {
...
Parse.Cloud.define("requestedPostList", async (request) => {
    var appId = request.headers['app-id']
    if (!appId) {
        throw "please supply appId"
    }else{
